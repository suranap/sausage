...
(define gentemp-cell (cons #f #f))
(define gentemp (lambda () ((car gentemp-cell))))
(define match:error
  (lambda (val . args)
    (for-each
     (lambda (x)
       (pretty-print x (current-output-port) 0)
       (newline))
     args)
			  ...
			  (cond
			   ...
			   ((memq match:error-control '(error fail)) (cons '()
									   (lambda (x)
									     `(match:error
									       ,x))))
			   ((eq? match:error-control 'match) (let ((errf (gentemp))
								   (arg (gentemp)))
							       (cons `((,errf
									(lambda (,arg)
									  (match:error
	   ...
	   (assm (lambda (tst f s)
		   (cond
		    ...
		    ((and (pair? s)
			  ...
			  (equal?
			   f
			   (cadar (cddadr (caadar (cddadr s)))))) (let ((k (car (cadadr
										 s)))
									(fail (car (caadar
										    (cddadr
										     s))))
