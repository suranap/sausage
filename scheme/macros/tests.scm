; A short test suite.
; Uses P and EVAL, neither of which is standard Scheme.
...
(define (evaluate expr) (eval expr (scheme-report-environment 5)))
...
(define (g answer)
  ...
  (set! if1     if0)
  (set! set!1   set!0)
  (if (not (equal? (evaluate (macro-expand original-code)) answer))
      (begin (newline)
             (display "TEST FAILED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
             (newline)
