...
;            ((eq? keyword denotation-of-let)           (m-let exp env))
            ((eq? keyword denotation-of-letrec)        (m-letrec exp env))
            ((eq? keyword denotation-of-case-lambda)   (m-case-lambda exp env))
...
(define (m-letrec exp env)
  ;; (letrec ((v1 b1) ...) body ...)
  (if (> (safe-length exp) 2)
      (let* ((bindings (cadr exp))
	     (names (map car bindings))
	     (alist (rename-vars names))
	     (env (syntactic-rename env alist))
	     (newnames (map (lambda (n) (cdr (assq n alist))) names))
	     (newbindings (map (lambda (b) (m-expand b env)) (map cdr bindings)))
	     (body (cddr exp)))
	(list letrec1 
	      (map cons newnames newbindings)
	      (m-expand body env)))
      (m-error "Malformed letrec expression" exp)))

(define (m-case-lambda exp env)
  ;; (case-lambda (f1 b1) (f2 b2) ...)  -- lambda w/ multiple entry points
  (if (> (safe-length exp) 2)
      (let ((conv-entry-point
	     (lambda (branch)
	       (cdr (m-lambda (cons lambda1 branch) env)))))
	(cons case-lambda1
	      (map conv-entry-point (cdr exp))))
      (if (= (safe-length exp) 2)	; really a lambda
	  (m-lambda (cons lambda1 (cadr exp)) env)
	  (m-error "Malformed lambda expression" exp))))
      ...
      (if (and (pair? exp)
          ...
          (let ((denotation (syntactic-lookup env (car exp))))
            (case (denotation-class denotation)
              ...
              ((macro)
	       (let ((keyword (syntactic-lookup env (car exp))))
		 (if (procedure? (macro-rules keyword))
		     (loop (cons (apply (macro-rules keyword) (cdr exp))
				 (cdr body))
			   env
			   defs)
		     (m-transcribe exp
...
(define (finalize-body body env defs)
  (if (null? defs)
      ...
      (let* ((alist (rename-vars '(quote lambda set!)))
             ...
             (new-letrec (cdr (assq 'letrec alist)))
...
(define (m-macro exp env)
  (let ((keyword (syntactic-lookup env (car exp))))
    (if (procedure? (macro-rules keyword))
	(m-expand (apply (macro-rules keyword) (cdr exp)) env)
	(m-transcribe exp
		      env
		      (lambda (exp env)
			(m-expand exp env))))))
